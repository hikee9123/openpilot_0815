...
void OnPaint::bb_ui_draw_measures_left(QPainter &p, int bb_x, int bb_y, int bb_w ) 
{
  ...
  if( false )
  {
   // float angleSteersDes = scene->controls_state.getSteeringAngleDesiredDegDEPRECATED();  
    ...
    //text3.sprintf("BF:%.1f   RL:%.1fÂ°", , scene->scr.accel_prob[1] );
...
void OnPaint::ui_draw_stop_sign( QPainter &p ) 
{
  ...
  cereal::LiveMapData::turnSpeedLimitsAhead::Reader  *pturnSpeedLimitsAhead  = osm.getTurnSpeedLimitsAhead();
//  float *pturnSpeedLimitsAheadDistances  = (float*) osm.getTurnSpeedLimitsAheadDistances();
